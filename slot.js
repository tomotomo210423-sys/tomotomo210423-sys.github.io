// === RETRO SLOT MACHINE - AUTO & PACHISLO EVOLUTION (Standalone Texture Edition) ===
const Slot={
  ...
  chkTen(){
    ...
    let s0 = stopped[0] ? this.getSyms(0) : null; let s1 = stopped[1] ? this.getSyms(1) : null; let s2 = stopped[2] ? this.getSyms(2) : null;
    let lines = this.getLines(s0, s1, s2);
    for (let l of lines) {
       let syms = l.s.filter(x => x !== null);
       if (syms[0] === syms[1]) return true;
       if (syms[0] === 'WLD' || syms[1] === 'WLD') { if (!['FRE','JP'].includes(syms[0]) && !['FRE','JP'].includes(syms[1])) return true; }
    }
    return false;
  },
  ...
  chkWin(){
    ...
    for(let l of lines){
      ...
      if(n.length===0){ this.win+=this.bet*100; this.lines.push(l.n); }
        ...
        let sym=n[0];
        if(sym==='FRE'||sym==='JP'){ if(n.length===3){ if(sym==='FRE')tF+=10; if(sym==='JP')tJ=true; this.lines.push(l.n); } } 
        else { this.win+=this.bet*this.pays[sym]; this.lines.push(l.n); }
      }
    }

    let usedSafety = false;
    if (this.win === 0 && this.activeItem === 'safety') { this.win += 30; usedSafety = true; }

    if(tJ){ this.win+=this.jp; this.jp=1000; this.msg='JACKPOT!!!'; SaveSys.addLog('スロット', 'ジャックポットを当てて大儲けした！'); }
    ...
    if (this.win > 0) {
        this.winCoins = []; let coinAmt = Math.min(50, this.win);
  ...
  update(){
    if(keysDown.select) { this.auto = !this.auto; playSnd('sel'); this.autoTmr = 0; }
        ...
        if(keysDown.a) {
           ...
           if (this.coins >= item.cost) {
               if (item.id === 'f_ticket') { this.coins -= item.cost; this.free += 10; playSnd('combo'); this.msg = 'GET 10 FREE SPINS!!'; } 
                   ...
                   if (this.coins >= 9999) { this.msg = 'COINS LIMIT REACHED!'; playSnd('hit'); } 
    ...
    if(this.st==='bet'){
      if(this.free>0){
        ...
        if(keysDown.a&&this.coins>=this.bet){ this.coins-=this.bet; this.jp+=this.bet; SaveSys.data.slotCoins=this.coins; SaveSys.data.jackpotPool=this.jp; SaveSys.save(); this.spin(); }
      }
    }
    else if(this.st === 'freeze') {
        if(this.tmr === 60) { playSnd('combo'); screenShake(15); this.msg = 'SYSTEM OVERRIDE!'; }
        if(this.tmr > 130) {
            for(let i=0; i<3; i++) { let targetIdx = this.lay[i].indexOf('JP'); let centerPos = (targetIdx - 1 + 20) % 20; this.reels[i].p = centerPos * this.symH; this.reels[i].st = true; this.reels[i].b = 10; }
      ...
      if (targetReel !== -1) {
        ...
        if (this.targetSym) {
            ...
            if (this.stoppedCount < 2) { r.p = centerPos * this.symH; } else { this.slipCount = Math.floor(Math.random() * 3) + 1; r.p = ((centerPos - this.slipCount + 20) % 20) * this.symH; }
        ...
        else if(this.stoppedCount>=3){ if(this.targetSym && this.slipCount > 0) { this.st = 'nudge'; this.tmr = 0; } else { this.st='pay'; this.tmr=0; this.chkWin(); } }
      }
    }
      ...
      if(this.rTmr>100){ 
        let targetReel = this.reels.findIndex(x=>!x.st); let r=this.reels[targetReel]; r.st=true; r.s=0;
        if(this.targetSym) {
            let targetIdx = this.lay[targetReel].indexOf(this.targetSym); let centerPos = (targetIdx - 1 + 20) % 20;
        ...
        if (this.tmr > 20 && this.tmr % 15 === 0) {
            ...
            r.p += this.symH; if (r.p >= 20 * this.symH) r.p -= 20 * this.symH; r.b = 5; this.slipCount--; playSnd('hit'); this.msg = 'NUDGE...! (HACKING)'; screenShake(3);
  ...
  draw(){
    const bgGrad = ctx.createLinearGradient(0,0,0,300); bgGrad.addColorStop(0, '#100'); bgGrad.addColorStop(1, '#301'); ctx.fillStyle = bgGrad; ctx.fillRect(0,0,200,300);

    let cabG = ctx.createLinearGradient(10,10,190,10); cabG.addColorStop(0,'#333'); cabG.addColorStop(0.5,'#555'); cabG.addColorStop(1,'#333'); ctx.fillStyle = cabG; ctx.fillRect(10,10,180,280);
    ctx.strokeStyle = '#111'; ctx.lineWidth = 4; ctx.strokeRect(10,10,180,280);

    let t = Date.now()/100;
    for(let i=0; i<14; i++) {
        let lx = 16 + i*12.8; ctx.fillStyle = (i%3 === Math.floor(t)%3) ? '#f00' : '#400';
        ctx.beginPath(); ctx.arc(lx, 55, 2, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(lx, 185, 2, 0, Math.PI*2); ctx.fill();
    }
    for(let i=0; i<10; i++) {
        let ly = 65 + i*12.2; ctx.fillStyle = (i%3 === Math.floor(t)%3) ? '#f00' : '#400';
        ctx.beginPath(); ctx.arc(16, ly, 2, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(184, ly, 2, 0, Math.PI*2); ctx.fill();
    }

    if (this.st === 'shop') {
        ctx.fillStyle='rgba(0,10,0,0.9)'; ctx.fillRect(15, 20, 170, 260); ctx.fillStyle='#0f0'; ctx.font='bold 12px monospace'; ctx.fillText('-- BLACK MARKET --', 30, 45); ctx.font='10px monospace';
        for(let i=0; i<this.shopData.length; i++) {
            let itm = this.shopData[i]; ctx.fillStyle = this.shopCur === i ? '#ff0' : '#0f0';
            ctx.fillText((this.shopCur === i ? '▶ ' : '  ') + itm.name, 25, 70 + i*22);
            ctx.fillStyle = this.coins >= itm.cost || itm.cost === 0 ? '#0ff' : '#050'; if (itm.cost > 0) ctx.fillText(itm.cost + 'G', 145, 70 + i*22);
        }
        ctx.fillStyle = '#080'; ctx.fillRect(20, 210, 160, 1); ctx.fillStyle = '#fff'; ctx.font='9px monospace'; ctx.fillText(this.shopData[this.shopCur].desc, 25, 230);
        ctx.fillStyle = '#888'; ctx.font = '8px monospace'; ctx.fillText('A:BUY / B,◀:EXIT', 50, 255);
    } else {
        if (this.st === 'bet') {
            ctx.fillStyle = '#ff0'; ctx.fillRect(180, 120, 10, 40); ctx.fillStyle = '#000'; ctx.font = '8px monospace';
            ctx.fillText('S', 182, 130); ctx.fillText('H', 182, 138); ctx.fillText('O', 182, 146); ctx.fillText('P', 182, 154);
            if(Math.floor(Date.now()/500)%2===0) ctx.fillText('▶', 182, 164);
        }

        ctx.fillStyle = '#000'; ctx.fillRect(25, 20, 150, 28); ctx.fillStyle = '#100'; for(let y=22;y<48;y+=2) ctx.fillRect(25,y,150,1); 
        ctx.shadowBlur = 5; ctx.shadowColor = '#f00'; ctx.fillStyle = '#f00'; ctx.font = 'bold 12px monospace'; ctx.fillText(`★ JACKPOT: ${this.jp}`, 30, 40);
        if (this.auto) { ctx.fillStyle = '#f00'; ctx.fillRect(25, 23, 30, 10); ctx.fillStyle = '#fff'; ctx.font = '8px monospace'; ctx.fillText('AUTO', 28, 31); } ctx.shadowBlur = 0;
        ...
        let shadow = ctx.createLinearGradient(0,62,0,178); shadow.addColorStop(0, 'rgba(0,0,0,0.8)'); shadow.addColorStop(0.15, 'rgba(0,0,0,0)'); shadow.addColorStop(0.85, 'rgba(0,0,0,0)'); shadow.addColorStop(1, 'rgba(0,0,0,0.8)'); ctx.fillStyle = shadow; ctx.fillRect(25,62,150,116);
        ctx.fillStyle = '#333'; ctx.fillRect(73, 62, 4, 116); ctx.fillRect(123, 62, 4, 116); ctx.fillStyle = '#ccc'; ctx.fillRect(74, 62, 2, 116); ctx.fillRect(124, 62, 2, 116);

        if (this.st === 'freeze') {
            ctx.fillStyle = (this.tmr < 60) ? 'rgba(0,0,0,0.9)' : ((this.tmr%4<2) ? 'rgba(255,0,0,0.6)' : 'rgba(0,0,255,0.6)'); ctx.fillRect(25,62,150,116);
        ...
        let linesY = [78, 120, 162]; linesY.forEach(y => { ctx.beginPath();ctx.moveTo(25,y);ctx.lineTo(175,y);ctx.stroke(); });
        ctx.beginPath();ctx.moveTo(25,78);ctx.lineTo(175,162);ctx.stroke(); ctx.beginPath();ctx.moveTo(25,162);ctx.lineTo(175,78);ctx.stroke();
        ...
        ctx.fillStyle = '#000'; ctx.fillRect(25, 192, 150, 50); ctx.fillStyle = '#100'; for(let y=194;y<240;y+=2) ctx.fillRect(25,y,150,1);
        ctx.fillStyle = '#0f0'; ctx.font = '11px monospace'; ctx.fillText(`CREDIT ${this.coins}`, 30, 208); ctx.fillStyle = '#ff0'; ctx.fillText(`BET ${this.bet}`, 125, 208);
        
        if (this.activeItem) { ctx.fillStyle='#0ff'; ctx.font='10px monospace'; ctx.fillText(`[${this.shopData.find(i=>i.id===this.activeItem).name} READY]`, 30, 225); } 
        else { ctx.fillStyle = (this.st==='pay' && this.win>0) ? '#ff0' : (this.free>0 ? '#0ff' : '#fff'); ctx.font='9px monospace'; ctx.fillText(this.msg, 30, 225); }
        
        ctx.fillStyle = '#0f0'; ctx.font = '9px monospace'; 
        if (this.st === 'spin') { ctx.fillText('◀:L  ▼:C  ▶:R  A:AUTO', 28, 238); } else if (this.st === 'bet' && this.tmr%40<20) { ctx.fillText('PRESS A TO PLAY', 30, 238); }

        ctx.fillStyle='#111'; ctx.fillRect(20,250,160,30); ctx.fillStyle='#fff'; ctx.font='8px monospace';
        ctx.fillText('7:x50 BAR:x20 BEL:x10 SUI:x5 CHE:x2', 23, 260); ctx.fillStyle='#f8f'; ctx.fillText('[W]:WILD', 23, 273);
        ctx.fillStyle='#0ff'; ctx.fillText('[F]:FREE', 75, 273); ctx.fillStyle='#ff0'; ctx.fillText('[王冠]:JACKPOT', 120, 273);

        if (this.winCoins.length > 0) { ctx.fillStyle = '#ff0'; ctx.strokeStyle = '#a80'; ctx.lineWidth = 1; for(let c of this.winCoins) { ctx.beginPath(); ctx.arc(c.x, c.y, 4, 0, Math.PI*2); ctx.fill(); ctx.stroke(); } }
    }
  }
};
