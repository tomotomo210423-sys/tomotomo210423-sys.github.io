// === MICRO QUEST - ULTIMATE SYMBOL ENCOUNTER VERSION ===
const RPG = {
  ...
  fieldMonsters: [], fightingMonsterIdx: -1, // シンボルエンカウント用
  spells: [ 
    {name: 'ファイア', mp: 5, dmg: 15, type: 'atk', desc: '敵に火炎で攻撃する'}, 
    {name: 'サンダー', mp: 8, dmg: 25, type: 'atk', desc: '激しい雷を落とす'}, 
    {name: 'ブリザド', mp: 12, dmg: 35, type: 'atk', desc: '全てを凍てつかせる'}, 
    {name: 'ヒール', mp: 10, dmg: -30, type: 'heal', desc: '自身のHPを回復する'}, 
    {name: 'ポイズン', mp: 8, dmg: 0, type: 'poison', desc: '敵を毒状態にする'}, 
    {name: 'ドレイン', mp: 15, dmg: 20, type: 'drain', desc: '敵のHPを吸収する'} 
  ],
  monsterTypes: { 
    slime: {name: 'スライム', spr: sprs.slime, c: '#0a0', hp: 15, atk: 3, def: 1, spd: 3, exp: 15, gld: 8, spells: []}, 
    bat: {name: 'コウモリ', spr: sprs.enemyNew, c: '#80f', hp: 20, atk: 5, def: 1, spd: 8, exp: 20, gld: 12, spells: [5]}, 
    skeleton: {name: '骨戦士', spr: sprs.skull, c: '#aaa', hp: 35, atk: 7, def: 3, spd: 6, exp: 30, gld: 20, spells: []}, 
    mage: {name: '魔道士', spr: sprs.mage, c: '#60c', hp: 25, atk: 10, def: 2, spd: 5, exp: 35, gld: 25, spells: [0, 1, 4]}, 
  ...
  genMap() {
    ...
    for(let i = 0; i < numMonsters; i++) {
       let rx = Math.floor(Math.random() * 10), ry = Math.floor(Math.random() * 15);
  ...
  exitDungeon() { this.map = this.worldMap; this.p.x = this.p.worldX; this.p.y = this.p.worldY; this.st = 'map'; BGM.play('rpg_field'); this.genMap(); },
