// === BLOCK CRAFT (3D) - INVENTORY & RESIZE FIX ===
const Mine = {
  st: 'init',
  ...
  // ★ 新規：インベントリ管理
  inventory: new Array(36).fill(null), // 0~8がホットバー
  selectedSlot: 0,
  ...
  init() {
    ...
    // 初期アイテム
    this.inventory[0] = { type: 'dirt', count: 10 };
    this.inventory[1] = { type: 'wood', count: 5 };
    this.updateHotbarUI();
  ...
  placeBlock() {
    ...
    if (!item || item.count <= 0) return; // 持っていない場合は置けない
    ...
    if (nx + 0.5 > px - pSize && nx - 0.5 < px + pSize && nz + 0.5 > pz - pSize && nz - 0.5 < pz + pSize && ny + 0.5 > py && ny - 0.5 < py + pHeight) return; 
    
    // 設置して、インベントリの数を減らす
    this.addBlock(nx, ny, nz, item.type);
    item.count--;
  ...
  spawnDrop(type, x, y, z) {
    ...
    if (type === 'coal') dropType = 'coal'; // 今はブロックの見た目として落とす
  ...
  addItem(type, amount) {
    for(let i=0; i<36; i++) {
        if(this.inventory[i] && this.inventory[i].type === type && this.inventory[i].count < 64) {
            this.inventory[i].count += amount; this.updateHotbarUI(); return;
        }
    }
    for(let i=0; i<36; i++) {
        if(!this.inventory[i]) {
            this.inventory[i] = { type: type, count: amount }; this.updateHotbarUI(); return;
        }
    }
  },
  ...
  // ★ ホットバーの見た目を更新する
  updateHotbarUI() {
    // 種類からテクスチャの座標(%)を計算する辞書
    const uvMap = {
        grass: {x:0, y:0}, dirt: {x:66.6, y:0}, stone: {x:100, y:0},
        cobblestone: {x:0, y:33.3}, wood: {x:33.3, y:33.3}, leaves: {x:100, y:33.3},
        plank: {x:0, y:66.6}, sand: {x:33.3, y:66.6}, glass: {x:66.6, y:66.6}, diamond: {x:100, y:66.6},
        iron: {x:0, y:100}, coal: {x:33.3, y:100}
    };
    ...
    for(let i=0; i<9; i++) {
        ...
        if(!slot) continue;
        slot.innerHTML = '';
        slot.className = i === this.selectedSlot ? 'hotbar-slot active' : 'hotbar-slot';
        
        const item = this.inventory[i];
        if (item && item.count > 0) {
            const icon = document.createElement('div');
            icon.style.width = '100%'; icon.style.height = '100%';
            icon.style.backgroundImage = "url('atlas.png')";
            icon.style.backgroundSize = "400% 400%";
            icon.style.imageRendering = "pixelated";
            const pos = uvMap[item.type] || {x:100, y:0};
            icon.style.backgroundPosition = `${pos.x}% ${pos.y}%`;
            
            const txt = document.createElement('div');
            txt.style.position = 'absolute'; txt.style.bottom = '2px'; txt.style.right = '2px';
            txt.style.color = '#fff'; txt.style.fontSize = '12px'; txt.style.textShadow = '1px 1px 0 #000'; txt.style.fontWeight = 'bold';
            txt.innerText = item.count > 1 ? item.count : '';
            
            slot.appendChild(icon); slot.appendChild(txt);
        }
    }
  },
  ...
  bindEvents() {
    ...
    touchZone.addEventListener('pointerdown', (e) => {
        ...
        e.preventDefault();
        touchZone.setPointerCapture(e.pointerId); this.touchZone.active = true; this.touchZone.pointerId = e.pointerId;
        this.touchZone.lastX = e.clientX; this.touchZone.lastY = e.clientY; this.touchZone.startT = Date.now(); this.touchZone.moved = false;
        this.digging = true;
    });
    ...
    touchZone.addEventListener('pointermove', (e) => {
        ...
        this.controls.yaw -= dx * 0.005; this.controls.pitch -= dy * 0.005; 
  ...
  // ★ 虚無空間回避：ピクセル単位で確実に強制リサイズ
  resize() {
    if (!this.camera || !this.renderer) return;
    const w = window.innerWidth; const h = window.innerHeight;
    this.camera.aspect = w / h; this.camera.updateProjectionMatrix(); this.renderer.setSize(w, h);
    
    // CSSの干渉を防ぐため、直接ピクセルを指定
  ...
  getCollidingBlock(nx, ny, nz, isWall = false) {
    const pSize = 0.3; const pHeight = 1.8; const footY = isWall ? ny + 0.1 : ny; 
    for (let b of this.blocks) {
      if (nx - pSize < b.x + 0.5 && nx + pSize > b.x - 0.5 && nz - pSize < b.z + 0.5 && nz + pSize > b.z - 0.5 && footY < b.y + 0.5 && ny + pHeight > b.y - 0.5) return b;
    }
    return null;
  },
  ...
  update() {
    ...
    for (let i = this.drops.length - 1; i >= 0; i--) {
        ...
        if (dist < 1.5) {
            ...
            // 拾ったらインベントリに加える
            this.addItem(d.type, 1);
            this.drops.splice(i, 1);
            if(typeof playSnd === 'function') playSnd('combo'); // 拾った音
        }
    }
  },
