// === ONLINE BATTLE - PACING & POSITION RESET UPDATE ===
const Online = {
  ...
  update() {
    ...
    if (this.st === 'wait') {
      this.tmr--;
      if (this.tmr <= 0) { 
        this.p2.n = ['Tanaka', 'xX_Pro_Xx', 'Gamer99', 'Pikachu', 'Bot_Alpha', 'Shadow', 'Nova', 'Blaze'][Math.floor(Math.random() * 8)]; 
        this.st = 'countdown'; this.countdown = 90; playSnd('combo'); this.resetB(); 
      }
    } 
    else if (this.st === 'countdown') { 
      // カウントダウン中もプレイヤーは準備のために動かせるようにしています
      if (keys.up) this.p1.y = Math.max(0, this.p1.y - 3);
      if (keys.down) this.p1.y = Math.min(260, this.p1.y + 3);
      
      this.countdown--; 
      if (this.countdown <= 0) { this.st = 'play'; } 
    }
      ...
      // ★ 点数が入った時の処理（固まらずに即座に次へ！）
      if (this.blt.x < 0) { this.p2.s++; this.onRoundEnd(); playSnd('hit'); screenShake(5); }
      else if (this.blt.x > 200) { this.p1.s++; this.onRoundEnd(); playSnd('combo'); screenShake(5); }
  ...
  // ラウンド終了時の処理をまとめました
  onRoundEnd() {
    if (this.p1.s >= 5 || this.p2.s >= 5) {
      this.st = 'result';
    } else {
      this.resetB(); // ボールを中央に
      this.p1.y = 130; // ★ 自分の位置を中央にリセット
      this.p2.y = 130; // ★ 相手の位置を中央にリセット
      this.st = 'countdown';
      this.countdown = 90; // カウントダウンを開始（1.5秒でテンポ良く次へ）
    }
  },
  ...
  draw() {
    ...
    if (this.st === 'wait') {
      ...
      // キャラクター(パドル)
      ctx.fillStyle = '#0f0'; ctx.fillRect(10, this.p1.y, 10, 40); 
      ctx.fillStyle = '#f00'; ctx.fillRect(180, this.p2.y, 10, 40);
      
      // ボールの残像と本体
      this.trail.forEach(t => { ctx.globalAlpha = t.life / 5; ctx.fillStyle = '#ff0'; ctx.fillRect(t.x, t.y, 8, 8); }); ctx.globalAlpha = 1;
      if (this.blt && (this.st === 'play' || this.st === 'countdown')) { 
        ctx.shadowBlur = 10; ctx.shadowColor = '#ff0'; ctx.fillStyle = '#ff0'; ctx.fillRect(this.blt.x, this.blt.y, 8, 8); ctx.shadowBlur = 0; 
      }
