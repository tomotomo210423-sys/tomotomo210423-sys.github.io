// === ONLINE BATTLE - ROYAL JOKER (Phase 5: Full Skills & Tempo Fix) ===
...
const Online = {
  ...
  setupConn() {
    ...
    this.conn.on('close', () => { this.st = 'error'; this.msg = 'CONNECTION LOST...'; playSnd('hit'); this.guestJoined = false; });
  },
  ...
  processAction(actionData, playerRole) {
    ...
    let myHand = this.state[playerRole + 'Hand'];
    let opHand = this.state[opRole + 'Hand'];
    ...
    if (actionData.type === 'draw') {
      ...
      myHand.push(drawnCard); playSnd('sel');
      
      this.state.activeSkill = 0; // 透視解除
      let matched = this.removePairs();
      
      this.state.msg = matched ? 'PAIR MATCHED!' : 'CARD DRAWN!';
      this.state.wait = 60; // ゆったりテンポ

      if (!this.checkWinOrShuffle()) {
         if (this.state.effects[playerRole].haste > 0) { // スキル9: 加速
             this.state.effects[playerRole].haste--; this.state.msg = 'HASTE: DRAW AGAIN!';
    ...
    else if (actionData.type === 'give') { // スキル3: 贈り物
      let card = myHand.splice(actionData.idx, 1)[0];
      opHand.push(card); opHand.sort(() => Math.random() - 0.5); // 相手の手札をシャッフル
  ...
  checkWinOrShuffle() {
    ...
    if (hLen === 0 || gLen === 0) {
      if (rev) this.state.msg = hLen === 0 ? 'GUEST WINS(REV)!!' : 'HOST WINS(REV)!!'; // 革命中は手札0で負け
      else this.state.msg = hLen === 0 ? 'HOST WINS!!' : 'GUEST WINS!!';
      this.state.wait = 9999; playSnd('combo'); return true;
    }
    ...
    if (this.state.wait > 0) {
      ...
      if (this.state.wait === 0) {
        if (this.state.msg === '⚠️ CHAOS SHUFFLE ⚠️') {
          ...
          if (this.state.effects[opRole].shield) { // スキル5: 鉄壁のブロック
  ...
  draw() {
    ...
    if (this.st === 'play') {
      ...
      for (let i = 0; i < opHand.length; i++) {
        ...
        if (this.state.turn === this.role && this.cursor === i && !this.isSkillMenu && this.state.pendingSkill !== 3 && this.state.wait === 0) {
          ctx.strokeStyle = '#0f0'; ctx.lineWidth = 2; ctx.strokeRect(x-2, y-6, 29, 39); ctx.lineWidth = 1;
          // スキル6: 予言（自分の手札に同じ数字があればO、なければXを表示）
      ...
      for (let i = 0; i < myHand.length; i++) {
        ...
        // スキル3: 贈り物ターゲットカーソル
        if (this.state.turn === this.role && this.cursor === i && this.state.pendingSkill === 3 && this.state.wait === 0) {
      ...
      if (this.isSkillMenu) {
        ...
        mySkills.forEach((sId, idx) => {
          ...
          ctx.fillText((this.skillCursor===idx?'>':' ') + S_NAMES[sId], 25 + idx*70, 200);
        });
      }
    }
    else {
