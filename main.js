...
const DataBackup = {
  ...
  draw() {
    ...
    if (this.st === 'msg') { 
        ctx.fillStyle = '#000'; ctx.fillRect(10, 150, 180, 80); ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; ctx.strokeRect(10, 150, 180, 80); 
        ctx.fillStyle = '#fff'; ctx.font = '10px monospace'; let arr = this.msg.split('\n'); for(let i=0; i<arr.length; i++) ctx.fillText(arr[i], 20, 170 + i*15); 
    }
  }
};
