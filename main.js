// === CORE SYSTEM - 16x16 FULL COLOR UPDATE ===
...
// ★ フルカラーパレット定義
const PALETTE = {
  '2': '#fff', '3': '#000', '4': '#fcc', '5': '#f00', '6': '#0a0', '7': '#00f', 
  '8': '#ff0', '9': '#842', 'a': '#aaa', 'b': '#0ff', 'c': '#f0f', 'd': '#80f'
};

// ★ 超強化：多色対応＆16x16自動スケーリング描画関数
const drawSprite = (x, y, color, str, size = 2.5) => {
  ...
  for (let i = 0; i < str.length; i++) {
    const char = str[i];
    if (char === '0') continue;
    // '1'の部分は引数の色(color)を使い、それ以外はパレットの色を使う
...
// ★ 超強化：フルカラー化スプライトデータ
// 1 の部分は着せ替えや闘技場で指定した色に変わるメインカラーです
const sprs = {
  hero: "",
