...
const bgThemes = [
  ...
  // ★ 新規追加: レトロなゲーム機風背景
  { name: 'GAMEBOY', draw: (ctx) => { 
      ctx.fillStyle = '#8bac0f'; ctx.fillRect(0, 0, 200, 300); 
      ctx.strokeStyle = '#9bbc0f'; ctx.lineWidth = 1;
      for (let i = 0; i < 200; i += 4) { ctx.beginPath(); ctx.moveTo(i, 0); ctx.lineTo(i, 300); ctx.stroke(); }
      for (let i = 0; i < 300; i += 4) { ctx.beginPath(); ctx.moveTo(0, i); ctx.lineTo(200, i); ctx.stroke(); }
      ctx.fillStyle = '#306230'; ctx.fillRect(0, 0, 6, 300); ctx.fillRect(194, 0, 6, 300); ctx.fillRect(0, 0, 200, 6); ctx.fillRect(0, 294, 200, 6);
  } }
];
...
function loop() {
  try {
    ...
    ctx.fillStyle = "#fff"; ctx.font = "10px monospace"; ctx.fillText("ERROR CRASHED", 10, 50); ctx.fillText(err.message.substring(0, 30), 10, 70);
  }
