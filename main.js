...
const SaveSys = {
  data: (() => { 
    ...
    return { playerName: d.playerName || 'PLAYER', scores: d.scores || { n: 0, h: 0 }, rankings: d.rankings || { n: [], h: [] }, bgTheme: d.bgTheme || 0, slotCoins: d.slotCoins || 100, jackpotPool: d.jackpotPool || 1000 }; 
  })(),
...
const Menu = {
  ...
  update() {
    ...
    if (keysDown.a) { const appObjs = [Guide, Tetri, Action, Online, Rhythm, Slot, Ranking, Settings, DataBackup]; switchApp(appObjs[this.cur]); }
  },
  draw() {
...
const DataBackup = {
  ...
  update() {
    ...
    if (keysDown.a) {
      if (Math.abs(this.px - 2) < 1.5 && Math.abs(this.py - 2) < 1.5) { try { const backupStr = btoa(unescape(encodeURIComponent(JSON.stringify(SaveSys.data)))); prompt("以下の呪文をコピーして保存してください:", backupStr); this.msg = '呪文を表示したぞ！'; this.st = 'msg'; playSnd('combo'); } catch(e) { this.msg = 'エラー発生'; this.st = 'msg'; } } 
      else if (Math.abs(this.px - 7) < 1.5 && Math.abs(this.py - 2) < 1.5) { const input = prompt("復活の呪文（パスワード）を入力してください:"); if (input) { try { const parsed = JSON.parse(decodeURIComponent(escape(atob(input)))); if (parsed && parsed.playerName) { SaveSys.data = parsed; SaveSys.save(); alert("復元成功！再起動します。"); location.reload(); } else throw new Error('Invalid'); } catch(e) { this.msg = '呪文が違います！'; this.st = 'msg'; playSnd('hit'); } } }
    }
  },
  draw() {
