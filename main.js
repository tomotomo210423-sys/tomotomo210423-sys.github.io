...
const BGM = {
  ...
  play(type) {
    ...
    const melodies = { menu: [262,294,330,349,392,349,330,294], tetri: [523,587,659,698,784,698,659,587], action: [392,440,494,523,494,440,392,349], rpg: [330,349,392,440,392,349,330,294] };
    const melody = melodies[type] || melodies.menu; const waveTypes = {menu:'square', tetri:'sine', action:'triangle', rpg:'sawtooth'}; const intervals = {menu:300, tetri:200, action:250, rpg:350};
    let i = 0;
...
function playSnd(t) {
  if (!audioCtx) return;
  const osc = audioCtx.createOscillator(); const gn = audioCtx.createGain(); osc.connect(gn); gn.connect(audioCtx.destination); const n = audioCtx.currentTime;
...
function drawParticles() {
  particles.forEach(p => { ctx.globalAlpha = p.life / 30; ctx.fillStyle = p.color; ctx.fillRect(p.x, p.y, p.size, p.size); ctx.globalAlpha = 1; });
}
...
const Menu = {
  ...
  draw() {
    ...
    ctx.fillStyle = '#fff'; ctx.font = '9px monospace'; ctx.fillText('ULTIMATE v4.0', 60, 45);
