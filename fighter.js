// === AUTO FIGHTER (Phase 21: SANS & DEVIL EDITION) Part 1 ===
...
const Styles = {
    ...
    AERO: { name: '空の支配者', desc: '常に空中戦を好む。ジャンプを多用し、上空からの強襲を狙うスタイル。', aggro: 0.6, guard: 0.1, dodge: 0.4, range: 60 },
    BALANCE: { name: 'バランス', desc: '近・遠・防御を状況に応じて使い分ける万能スタイル。', aggro: 0.5, guard: 0.3, dodge: 0.3, range: 80 }, // ★新規追加
    DEVIL: { name: 'デビル', desc: '相手をハメることに全力を尽くす。コンボと追撃を最優先する極悪スタイル。', aggro: 1.0, guard: 0.0, dodge: 0.1, range: 45 }, // ★新規追加
    SANS: { name: 'Sans', desc: '【特殊】HPが回避ゲージとなり確定回避。攻撃力は低いがカルマ(毒)で削り取る。', aggro: 0.4, guard: 0.0, dodge: 1.0, range: 150 } // ★新規追加
};
...
const Passives = {
    ...
    SANS_DODGE: { name: 'オート回避', desc: 'Sans専用。HPを消費して攻撃を確定回避。尽きると一撃で倒される。' } // ★新規追加
};
...
const AwakenPassives = {
    ...Passives, 
    CLONE: { name: '影分身', desc: '覚醒時、自身と同じ動きをする分身を2体召喚する。分身は一度被弾すると消滅する。' }
};
...
const Skills = {
    ...
    smash: {name:'スマッシュ', dmg: 40, kb: 12, range: 45, start: 14, act: 15, rec: 35, cd: 40, vx: 8, type: 'melee', desc: '相手を強烈に吹き飛ばす必殺の一撃。'},
    ...
    beam: {name:'ビーム', dmg: 40, kb: 10, range: 350, start: 25, act: 15, rec: 40, cd: 60, vx: -2, type: 'range', desc: '相手を正確に狙い撃つ長距離レーザー。'},
    sonic: {name:'ソニック', dmg: 25, kb: 4, range: 200, start: 12, act: 1, rec: 25, cd: 30, vx: 0, type: 'shot', desc: '相手のいる方向へ飛んでいく衝撃波。'},
    dive: {name:'急降下', dmg: 35, kb: 8, range: 60, start: 8, act: 15, rec: 25, cd: 30, vx: 10, type: 'air', desc: '【空中専用】斜め下へ鋭く突進蹴り。'},
    ...
    physCounter: {name:'物理当て身', dmg: 0, kb: 0, range: 0, start: 2, act: 30, rec: 20, cd: 30, vx: 0, type: 'stance_phys', desc: '直接攻撃を無効化して超絶反撃。'},
    magReflect: {name:'魔法反射', dmg: 0, kb: 0, range: 0, start: 2, act: 40, rec: 20, cd: 30, vx: 0, type: 'stance_mag', desc: '飛び道具を相手に跳ね返す。'},
    parapara: {name:'パラパラ', dmg: 5, kb: 1, range: 0, start: 10, act: 5, rec: 20, cd: 90, vx: 0, type: 'summon', desc: '自機を追従し自動で弾を撃つUFOを召喚。'},
    pull: {name:'追撃腕', dmg: 15, kb: -15, range: 150, start: 8, act: 10, rec: 20, cd: 30, vx: 0, type: 'pull', desc: '腕を伸ばして相手を自分の手前へ引き寄せる。'},
    burst: {name:'爆裂拳', dmg: 6, kb: 1, range: 40, start: 6, act: 40, rec: 20, cd: 35, vx: 3, type: 'multi', desc: '連続パンチ。最後の一撃は超威力で吹き飛ばす。'},
    random: {name:'RANDOM', dmg: 0, kb: 0, range: 100, start: 5, act: 5, rec: 15, cd: 20, vx: 0, type: 'random', desc: '自爆か回復か謎の雷か...何が起こるか予測不可能。'},
    // ★ Sans専用スキル（通常は選べないが、Sansスタイル時に強制セット）
    bone_up: {name:'骨の壁', dmg: 1, kb: 0, range: 200, start: 10, act: 30, rec: 15, cd: 20, vx: 0, type: 'sans_bone', desc: '地面から連続して骨を生やす。'},
    g_blaster: {name:'ブラスター', dmg: 2, kb: 0, range: 350, start: 15, act: 15, rec: 25, cd: 35, vx: 0, type: 'sans_blaster', desc: 'ガスターブラスターを召喚し極太ビームを放つ。'},
    bone_throw: {name:'通常骨', dmg: 1, kb: 0, range: 250, start: 5, act: 15, rec: 10, cd: 15, vx: 0, type: 'sans_throw', desc: '様々なサイズの骨を大量に放つ。'},
    shortcut: {name:'ちかみち', dmg: 0, kb: 0, range: 0, start: 2, act: 5, rec: 5, cd: 30, vx: 0, type: 'sans_warp', desc: '一瞬で有利な位置へテレポートする。'}
};

const SkillKeys = Object.keys(Skills).filter(k => !k.startsWith('sans_')); // 通常選択できるスキルのみ
...
const AutoFighter = {
  ...
  sanitizeAI(ai) {
      ...
      // ★ Sansスタイルを選んだ時の強制セットアップ
      if (ai.styleKey === 'SANS') {
          ai.skillKeys = ['bone_up', 'g_blaster', 'bone_throw', 'shortcut'];
          ai.passiveKey = 'SANS_DODGE';
          ai.color.body = '#fff'; ai.color.aura = '#0ff'; // 骨と青目
      } else {
  ...
  drawDescBox(title, descStr) {
      ...
      ctx.fillStyle = '#ff0'; ctx.font = 'bold 11px monospace'; ctx.fillText(title || '[ INFO ]', 15, 220);
      ctx.fillStyle = '#fff'; ctx.font = '9px monospace'; let safeDesc = descStr || '';
      let lines = []; for(let i=0; i<safeDesc.length; i+=17) lines.push(safeDesc.substring(i, i+17));
      for(let i=0; i<lines.length; i++) ctx.fillText(lines[i], 15, 238 + i*13);
  },
  ...
  createFighter(isP2, data) {
      ...
      let safeBonus = data.bonus || {atk:0, res:0, spd:0}; let safeBase = data.base || {atk:1, res:1, spd:1};
      return {
          id: isP2 ? 2 : 1, x: isP2 ? 550 : 250, y: this.groundY, dir: isP2 ? -1 : 1, hp: 1000, maxHp: 1000, kr: 0, // ★ カルマ用変数
          ...
          hitCancel: false, hasHit: false, combo: 0, comboTimer: 0, comboDmg: 0, isAwakened: false, funnelTimer: 0, hasClones: false 
      };
  },

  setupSimBattle() {
      this.checkHealth(); this.timer = 0; this.texts = []; this.vfx = []; this.bullets = [];
      ...
      if (isMirror) { 
          ...
          // ★ 敵Botの多様化（スタイルに応じた技とパッシブを装備）
          let eStyle = Object.keys(Styles)[Math.floor(Math.random()*Object.keys(Styles).length)];
          let dummySkills = []; let dummyPassive = 'NONE';
          if (eStyle === 'ZONE') { dummySkills = ['beam', 'sonic', 'parapara', 'magReflect']; dummyPassive = 'HOVER'; }
          else if (eStyle === 'RUSH' || eStyle === 'DEVIL') { dummySkills = ['jab', 'smash', 'burst', 'throw']; dummyPassive = 'VAMPIRE'; }
          else if (eStyle === 'AERO') { dummySkills = ['dive', 'meteor', 'sonic', 'upper']; dummyPassive = 'HOVER'; }
          else if (eStyle === 'COUNTER') { dummySkills = ['physCounter', 'magReflect', 'upper', 'pull']; dummyPassive = 'MAHORAGA'; }
          else if (eStyle === 'SANS') { dummySkills = ['bone_up', 'g_blaster', 'bone_throw', 'shortcut']; dummyPassive = 'SANS_DODGE'; }
          else { dummySkills = [...SkillKeys].sort(()=>Math.random()-0.5).slice(0,4); dummyPassive = 'LEARNING'; }

          let dummyData = this.sanitizeAI({ name: 'DUMMY-' + eStyle, styleKey: eStyle, skillKeys: dummySkills, passiveKey: dummyPassive });
          if(eStyle === 'SANS') { dummyData.color = { body: '#fff', aura: '#0ff' }; }
  ...
  update() {
      try { this._update(); } catch(e) { console.error("Update Protected:", e); this.timer++; }
  },
  ...
  _update() {
    ...
    if (this.st === 'lab_main') {
        const labItems = ['AI名前変更', '戦闘スタイル設定', 'スキルセット(技)', 'パッシブ＆覚醒', 'ステータス配分', '体型＆カラー', '通常学習(20回)', '無限強化学習', 'AIデータ初期化', '戻る'];
        ...
        if (keysDown.a) {
            ...
            if (this.labCur === 0) { 
                // ★ 名前変更機能
                let newName = prompt("AIの名前を入力してください (10文字以内)", this.myAI.name);
                if (newName && newName.trim() !== '') { this.myAI.name = newName.substring(0, 10); this.play('combo'); }
            }
            else if (this.labCur === 1) { this.st = 'lab_style'; this.labCur = 0; }
            else if (this.labCur === 2) { this.st = 'lab_skills'; this.labCur = 0; }
            else if (this.labCur === 3) { this.st = 'lab_awaken'; this.labCur = 0; }
            else if (this.labCur === 4) { this.st = 'lab_stats'; this.labCur = 0; }
            else if (this.labCur === 5) { this.st = 'lab_body'; this.labCur = 0; }
            else if (this.labCur === 6) { this.isSim = true; this.isInfinite = false; this.simEpoch = 0; this.simMaxEpoch = 20; this.simWins = 0; this.setupSimBattle(); this.st = 'training'; this.trainingMsg = '仮想敵 生成完了...'; BGM.play('action'); }
            else if (this.labCur === 7) { this.isSim = true; this.isInfinite = true; this.simEpoch = 0; this.simWins = 0; this.setupSimBattle(); this.st = 'training'; this.trainingMsg = '※Bボタンでいつでも中断・保存できます'; BGM.play('action'); }
            else if (this.labCur === 8) { this.myAI = this.sanitizeAI({}); this.texts = []; this.addText(100, 150, "AI INITIALIZED!", "#f00"); }
    ...
    if (this.st === 'training') {
        ...
        for(let loop = 0; loop < 10; loop++) {
            ...
            if (this.p1.hp <= 0 || this.p2.hp <= 0 || this.timer > 1500) {
                ...
                this.myAI.bonus.atk = Math.min(5.0, safeNum(this.myAI.bonus.atk, 0) + b);
                this.myAI.bonus.res = Math.min(5.0, safeNum(this.myAI.bonus.res, 0) + b);
                this.myAI.bonus.spd = Math.min(5.0, safeNum(this.myAI.bonus.spd, 0) + b);
  ...
  applyHit(attacker, skill, victim, hx, hy) {
       ...
       let isSansAtk = attacker.styleKey === 'SANS';
       let isSansDef = pKeyV === 'SANS_DODGE';

       if (victim.hasClones) {
           victim.hasClones = false; this.addVFX('shockwave', victim.x, victim.y, '#fff', {size: 80});
           this.addText(victim.x, victim.y - 40, "身代わり!", "#fff"); this.play('hit'); return true;
       }
       ...
       if (safeNum(victim.comboDmg,0) > 100 || safeNum(attacker.combo,0) >= 8) {
       ...
       if (victim.state === 'atk_physCounter' && skill.type !== 'shot' && skill.type !== 'range' && !isSansAtk) {
       ...
       let safeAtk = safeNum(attacker.base && attacker.base.atk, 1.0);
       let damage = safeNum(skill.dmg, 10) * safeAtk; 
       let aHp = safeNum(attacker.hp, 1000); let aMax = Math.max(1, safeNum(attacker.maxHp, 1000));
       
       if (pKeyA === 'DESPERATION' && (aHp / aMax) <= 0.3) damage *= 1.5;
       ...
       // ★ Sansのオート回避（HPを消費して確定回避）
       if (isSansDef && skill.type !== 'throw') {
           let dodgeCost = damage * 1.5; // ダメージ以上の体力を消耗する
           if (victim.hp > dodgeCost + 10) {
               victim.hp -= dodgeCost; 
               this.addText(victim.x, victim.y - 40, "MISS", "#ccc");
               victim.x += (Math.random() < 0.5 ? -60 : 60); // テレポート回避
               victim.x = Math.max(10, Math.min(this.stageWidth - 10, victim.x));
               this.play('sel'); return true;
           } else {
               damage = 9999; // 体力が尽きたら一撃必殺
           }
       }

       // ★ Sansの攻撃特性（ダメージ1、ノックバック0、カルマ付与）
       let kbForce = safeNum(skill.kb, 2) * safeAtk * (100 / safeWeight);
       if (isSansAtk) {
           victim.kr = safeNum(victim.kr, 0) + damage * 2; // カルマ（毒）蓄積
           damage = 1; kbForce = 0; // 怯み・ノックバックなしの多段ヒット
       }
       ...
       if (isBurstFinish) { damage *= 5; kbForce = 20 * safeAtk * (100 / safeWeight); }
       if (skill.type === 'pull') kbForce = -15; // 追撃腕は引き寄せる

       if (pKeyV === 'MAHORAGA' && !isSansAtk) {
           if (!victim.hitHistory) victim.hitHistory = {}; if (!victim.adapting) victim.adapting = {};
       ...
       victim.hp -= damage; 
       if (pKeyA === 'VAMPIRE' && !isSansAtk) attacker.hp = Math.min(aMax, aHp + damage * 0.2);
       ...
       if (!isSansAtk) {
           ...
           else if (safeNum(skill.kb, 0) > 10 || isBurstFinish) victim.vy = -8 - (Math.random()*2); 
           victim.vx = attacker.dir * kbForce;
       }
       
       let auraCol = (attacker.color && attacker.color.aura) ? attacker.color.aura : '#ff0';
       if (isSansAtk) auraCol = '#fff';
       
       if (sName === 'ジャブ' || isSansAtk) { this.play('hit'); this.shake(2); this.addVFX('impact', hx, hy, '#fff', {size: 20}); } 
       else { this.play('combo'); this.shake(10); this.stop(5); this.addVFX('impact', hx, hy, auraCol, {size: 50}); this.addVFX('slash', hx, hy, '#fff', {size: 40, angle: Math.random()*Math.PI*2, width: 4}); }
       
       return true;
  },

  createHitbox(attacker, skill, victim, offsetX = 0) {
    let ax = safeNum(attacker.x, 0) + offsetX; let ay = safeNum(attacker.y, 0);
    let vx = safeNum(victim.x, 0); let vy = safeNum(victim.y, 0);
    let aDir = safeNum(attacker.dir, 1); let aWidth = Math.max(0.1, safeNum(attacker.body && attacker.body.width, 1.0));
    
    let vDist = Math.hypot(vx - ax, vy - ay);
    let inFront = (vx - ax) * aDir >= -15; 
    let actualRange = safeNum(skill.range, 30) * aWidth;
    
    // 全方位攻撃（爆裂拳など）や引き寄せは後ろにも少し判定を持たせる
    if (skill.type === 'multi' || skill.type === 'pull' || skill.type.startsWith('sans_')) inFront = true;

    if (inFront && vDist <= actualRange + 15) { return this.applyHit(attacker, skill, victim, ax + ((vx-ax)/2), vy - 15); }
    return false;
  },
  ...
  processFighter(f, opp) {
    ...
    // ★ カルマ（毒）の継続ダメージ処理
    if (safeNum(f.kr, 0) > 0 && f.stateFrame % 3 === 0) {
        f.hp -= 1; f.kr -= 1;
        this.addVFX('impact', f.x + (Math.random()-0.5)*30, f.y - Math.random()*50, '#c0c', {size: 4, life: 5}); // 紫の毒エフェクト
    }

    if (f.funnelTimer > 0) {
        f.funnelTimer--;
        if (f.funnelTimer % 30 === 0) {
            let by = safeNum(f.y, this.groundY) - 60; let oy = safeNum(opp.y, this.groundY);
            let dx = safeNum(opp.x, 0) - safeNum(f.x, 0); let dy = oy - by;
            let dist = Math.hypot(dx, dy) || 1;
            this.bullets.push({x: safeNum(f.x, 0), y: by, vx: (dx/dist)*10, vy: (dy/dist)*10, owner: f, skill: {name:'パラ弾', dmg: 5, kb: 1, type: 'shot'}, life: 60});
            this.addVFX('impact', safeNum(f.x, 0), by, '#0ff', {size: 15, life: 5});
        }
    }
    ...
    f.stateFrame++; 
    let safeSpd = Math.max(0.5, safeNum(f.base && f.base.spd, 1.0));
    ...
    if (!f.isAwakened) {
        ...
        if (awaken) {
            ...
            if (f.awakenPassive === 'CLONE') f.hasClones = true; 
    ...
    if (pKey === 'LEARNING' && f.stateFrame % 60 === 0) {
        let oppStateStr = opp.state || '';
        if (oppStateStr.startsWith('atk_')) {
            let sObj = getSkill(oppStateStr.split('_')[1]);
            if (sObj) {
                if (sObj.type === 'shot' || sObj.type === 'range') f.dynDodge = Math.min(1.0, safeNum(f.dynDodge,0) + 0.05); 
                else f.dynGuard = Math.min(1.0, safeNum(f.dynGuard,0) + 0.05); 
    ...
    let isFloating = (pKey === 'HOVER' || f.styleKey === 'SANS'); // Sansも少し浮遊気味に動ける
    let fst = f.state || 'idle';
    if (isFloating && !['hurt', 'knockdown', 'stunned'].includes(fst)) {
        f.vx *= 0.85; f.vy *= 0.85; 
    } else {
        f.vx *= 0.85; 
    ...
    let fStateStr = f.state || '';
    if (Math.abs(f.vx) > 4 || Math.abs(f.vy) > 4 || fStateStr.startsWith('atk_')) { 
        if(!f.trail) f.trail = []; f.trail.unshift({x: f.x, y: f.y, dir: f.dir, state: f.state, frame: f.stateFrame}); 
        if(f.trail.length > 5) f.trail.pop(); 
    } else if (f.trail && f.trail.length > 0) { f.trail.pop(); }
    ...
    let isAtk = fStateStr.startsWith('atk_'); let sKey = isAtk ? fStateStr.split('_')[1] : ''; 
    ...
    if (isAtk && skill) {
         ...
         if (f.stateFrame === sFrame && sType === 'random') {
             ...
             else { let rndKey = SkillKeys[Math.floor(Math.random() * (SkillKeys.length - 4))]; f.state = 'atk_' + rndKey; f.stateFrame = 0; return; }
         }
         
         if (f.stateFrame === sFrame - 2 && (sType === 'warp' || sType === 'sans_warp')) { f.x = opp.x - opp.dir * (Math.random()*100 + 40); f.dir = opp.dir; this.addVFX('shockwave', f.x, f.y, '#ccc', {size: 30, life: 10}); }
         
         if (sType === 'multi' && f.stateFrame % 6 === 0) f.hasHit = false; // 爆裂拳の多段リセット
         if (sType === 'sans_bone' && f.stateFrame % 8 === 0 && f.stateFrame <= sFrame + actFrame) { f.hasHit = false; } // 骨の壁の多段リセット

         if (f.stateFrame >= sFrame && f.stateFrame <= sFrame + actFrame) { 
             let originX = f.x + 20*f.dir; let originY = f.y - 20 * Math.max(0.1, safeNum(f.body&&f.body.height, 1));
             let targetX = safeNum(opp.x, 250); let targetY = safeNum(opp.y, this.groundY) - 20 * Math.max(0.1, safeNum(opp.body&&opp.body.height, 1));
             let angleToOpp = Math.atan2(targetY - originY, targetX - originX);

             if (f.stateFrame === sFrame) {
                 if (sType === 'anti_air') { f.vy = -12; f.vx = f.dir * 4; }
                 if (sName === '急降下') { f.vy = 10; f.vx = f.dir * 12; }
                 if (sType === 'buff') { f.hp = Math.min(f.maxHp, f.hp + 200); this.addVFX('shockwave', f.x, f.y, '#0f0', {size: 50}); this.play('combo'); }
                 if (sType === 'summon') { f.funnelTimer = 180; this.play('jmp'); } 
                 
                 // ★ 飛び道具エイム
                 if (sType === 'shot') { this.play('jmp'); this.bullets.push({x: originX, y: originY, vx: Math.cos(angleToOpp)*12, vy: Math.sin(angleToOpp)*12, owner: f, skill: skill, life: 60}); } 
                 else if (sType === 'range') { this.addVFX('beam', originX, originY, (f.color?f.color.body:'#fff'), {size: safeNum(skill.range, 350), angle: angleToOpp, life: 15}); this.shake(6); }
                 
                 // ★ Sans専用技の描画と弾生成
                 if (sType === 'sans_blaster') {
                     this.addVFX('shockwave', originX, originY, '#fff', {size: 40});
                     this.bullets.push({x: originX, y: originY, vx: Math.cos(angleToOpp)*25, vy: Math.sin(angleToOpp)*25, owner: f, skill: skill, life: 30});
                     this.play('combo'); this.shake(15);
                 }
                 if (sType === 'sans_throw') {
                     for(let i=0; i<5; i++) { this.bullets.push({x: originX, y: originY - i*15, vx: f.dir*(10+Math.random()*5), vy: (Math.random()-0.5)*5, owner: f, skill: skill, life: 60}); }
                     this.play('jmp');
                 }
             ...
             if (!f.hasHit && sType !== 'buff' && sType !== 'shot' && sType !== 'range' && sType !== 'summon' && sType !== 'sans_blaster' && sType !== 'sans_throw' && !sType.startsWith('stance')) {
                 ...
                 if (sType === 'sans_bone') {
                     // 骨壁は足元から広範囲に判定
                     mainHit = this.createHitbox({...f, x: f.x + f.dir*100}, {...skill, range: 200}, opp, 0);
                     if(f.stateFrame % 8 === 0) this.addVFX('slash', f.x + f.dir*100, this.groundY, '#fff', {size: 80, angle: -Math.PI/2, width: 20});
                 } else {
                     mainHit = this.createHitbox(f, skill, opp, 0);
                 }

                 let cloneHit = false;
                 if (!mainHit && f.hasClones) { cloneHit = this.createHitbox(f, skill, opp, -40) || this.createHitbox(f, skill, opp, 40); }
                 
                 if (mainHit || cloneHit) {
                     f.hasHit = true; let angle = f.dir === 1 ? 0 : Math.PI; if (sType === 'anti_air') angle -= (Math.PI/4) * f.dir; if (sType === 'air') angle += (Math.PI/4) * f.dir; 
                     if (sName !== 'ジャブ' && sType !== 'sans_bone') this.addVFX('slash', originX, originY, (f.color?f.color.body:'#fff'), {size: safeNum(skill.range, 30) * Math.max(0.1, safeNum(f.body&&f.body.width, 1)), angle: angle, width: 10});
                 }
             }
         }
    ...
    if ((f.state === 'idle' && f.cd <= 0) || canCancel) {
        ...
        // ★ デビルスタイルは一切ガード・回避せず殴り続ける
        let dynG = st.name === 'デビル' ? 0 : safeNum(f.dynGuard, 0.2); 
        let dynD = st.name === 'デビル' ? 0 : safeNum(f.dynDodge, 0.2);
        ...
        if (opp.state === 'hurt' || opp.state === 'stunned' || opp.state === 'knockdown') {
            shouldAttack = true; 
            if (oppIsAir && !isAir && d < 60 && f.state !== 'move' && !isFloating) { f.state = 'move'; f.stateFrame = 0; f.vy = -16; f.vx = f.dir * 5; f.cd = 0; f.hitCancel = false; return; }
        } else {
            if ((st.name === '空の支配者' || f.styleKey === 'SANS') && !isAir && Math.random() < 0.2 && !canCancel && !isFloating) { f.state = 'move'; f.stateFrame = 0; f.vy = -18; f.vx = f.dir * 6; return; }
            if (d < safeNum(st.range, 50)) { if (Math.random() < safeNum(st.aggro, 0.5)) shouldAttack = true; else moveDir = -f.dir; } else { moveDir = f.dir; }
        }
        ...
        if (shouldAttack) {
            ...
            if (isFloating) {
                ...
                if (Math.abs(dy) > 20) { f.vy += (dy > 0 ? 1 : -1) * ninjaSpd; isMoving = true; }
                if (moveDir !== 0) { f.vx += moveDir * 1.5 * ninjaSpd; isMoving = true; }

                if (isMoving && f.state === 'idle') { f.state = 'move'; f.stateFrame = 0; }
            } else if (moveDir !== 0 && Math.random() < (safeSpd * 0.3)) { 
                f.state = 'move'; f.stateFrame = 0; f.vx = moveDir * 6 * ninjaSpd; 
            }
        }
    }
  ...
  drawStickman(f, alpha = 1, isTrail = false) {
    ...
    let drawCore = (dx, dy, myAlpha) => {
        ...
        if (fst === 'idle' || fst === 'move') { 
            ...
            if (isFloating) {
                ...
                else if (safeNum(f.y, this.groundY) < this.groundY - 5) { p.hL.y-=20; p.hR.y-=20; p.eL.y-=10; p.eR.y-=10; p.kL.y-=15; p.kR.y-=5; p.fL.y-=15; p.h.x+=5; } 
    ...
    if (!isTrail && passKey === 'MAHORAGA' && f.hitHistory) {
        ...
        if (maxAdapt > 0) {
            ...
            let rot = (sf * maxAdapt) * 0.1; ctx.beginPath();
            for(let i=0; i<8; i++){ ctx.moveTo(dx, dy-15*bH); ctx.lineTo(dx + Math.cos(rot+i*Math.PI/4)*10, dy-15*bH + Math.sin(rot+i*Math.PI/4)*10); } ctx.stroke();
        }
    }
    ctx.globalAlpha = 1;
    ...
    if (!isTrail && f.funnelTimer > 0) {
        ctx.fillStyle = cAura; ctx.shadowBlur = 10; ctx.shadowColor = cAura;
        let ufoY = dy - 30 + Math.sin(Date.now()/100)*5;
        ctx.fillRect(dx - 10, ufoY, 20, 4); ctx.fillRect(dx - 5, ufoY - 4, 10, 4);
        ctx.shadowBlur = 0;
    }
  },
  ...
  _draw() {
    if (this.st === 'menu' || this.st.startsWith('lab_') || this.st === 'save_slot' || this.st === 'load_slot') {
        ...
        if (this.st === 'menu') {
            ...
            ctx.fillStyle = '#888'; ctx.font = '8px monospace'; ctx.fillText(`[ ${this.myAI.name} ] Lv.${Math.floor(safeNum(this.myAI.learningLevel,0)/10)}`, 15, 25);
            ctx.fillText('NORMAL', 55, 35); ctx.fillText('AWAKENED', 130, 35);
            let safeC = this.myAI.color || {}; let safeAwkC = this.myAI.awakenColor || '#f00';
            let prevF = { x: 0, y: 0, dir: 1, state: 'idle', stateFrame: 0, body: this.myAI.body, color: safeC, isAwakened: false };
            ctx.save(); ctx.translate(50, 80); this.drawStickman(prevF); ctx.restore();
            prevF.isAwakened = true; prevF.color = {body: safeC.body||'#fff', aura: safeAwkC}; ctx.save(); ctx.translate(145, 80); this.drawStickman(prevF); ctx.restore();

            ctx.fillStyle = '#0f0'; ctx.font = 'bold 12px monospace';
            
            if (this.st === 'lab_main') {
                ctx.fillText('【CUSTOMIZE MENU】', 30, 110); ctx.font = '10px monospace';
                const items = ['AI名前変更', '戦闘スタイル設定', 'スキルセット(技)', 'パッシブ＆覚醒', 'ステータス配分', '体型＆カラー', '通常学習(20回)', '無限強化学習', 'AIデータ初期化', '戻る'];
                for(let i=0; i<items.length; i++) { ctx.fillStyle = this.labCur === i ? '#ff0' : '#fff'; ctx.fillText((this.labCur === i ? '> ' : '  ') + items[i], 20, 125 + i * 15); }
            }
                ...
                ctx.fillText('【SKILL SETTING】', 40, 115); ctx.font = '10px monospace'; 
                let sKeys = this.myAI.skillKeys || []; 
                if(this.myAI.styleKey === 'SANS') { ctx.fillStyle = '#888'; ctx.fillText("※Sans専用スキルで固定されています", 10, 135); }
                for(let i=0; i<4; i++) { ctx.fillStyle = this.labCur === i ? '#ff0' : '#fff'; let skName = getSkill(sKeys[i]).name; ctx.fillText(`SLOT ${i+1}: ◀ ${skName.padEnd(5,' ')} ▶`, 20, 150 + i * 15); } 
                let curSkill = getSkill(sKeys[this.labCur]); this.drawDescBox(`[ ${curSkill.name} ] 威力:${curSkill.dmg} CD:${curSkill.cd}`, curSkill.desc); 
            }
    ...
    this.vfx.forEach(v => {
        ...
        if (v.type === 'slash') { 
            ctx.strokeStyle = v.color; ctx.lineWidth = Math.max(0.1, safeNum(v.width,2) * ratio); ctx.lineCap = 'round'; ctx.beginPath(); 
            ctx.arc(safeNum(v.x,0), safeNum(v.y,0), vSize, safeNum(v.angle,0) - 1.0*ratio, safeNum(v.angle,0) + 1.0*ratio); ctx.stroke(); 
        } 
            ...
            ctx.lineTo(v.x, v.y + s); ctx.lineTo(v.x - s/4, v.y + s/4); ctx.lineTo(v.x - s, v.y); ctx.lineTo(v.x - s/4, v.y - s/4); ctx.fill(); 
        } 
        else if (v.type === 'shockwave') { 
            ctx.strokeStyle = v.color; ctx.lineWidth = Math.max(0.1, 4 * ratio); ctx.beginPath(); 
            ctx.arc(safeNum(v.x,0), safeNum(v.y,0), Math.max(0.1, vSize * Math.max(0, 1-ratio)*2 + 10), 0, Math.PI*2); ctx.stroke(); 
        } 
        else if (v.type === 'beam') { 
            ctx.save(); ctx.translate(safeNum(v.x,0), safeNum(v.y,0)); ctx.rotate(safeNum(v.angle,0));
            let h = Math.max(0.1, 20 * ratio); ctx.fillStyle = v.color; 
            ctx.fillRect(0, -h/2, vSize, h); ctx.fillStyle = '#fff'; ctx.fillRect(0, -h/6, vSize, h/3); 
            ctx.restore();
        }
        else if (v.type === 'hook') {
            ctx.strokeStyle = v.color; ctx.lineWidth = Math.max(0.1, 6 * ratio); ctx.lineCap = 'round'; ctx.beginPath();
            ctx.moveTo(safeNum(v.x,0), safeNum(v.y,0)); ctx.lineTo(safeNum(v.x,0) + (safeNum(v.targetX,0) - safeNum(v.x,0))*(1-ratio), safeNum(v.y,0) + (safeNum(v.targetY,0) - safeNum(v.y,0))*(1-ratio)); ctx.stroke();
        }
    });
    ctx.globalAlpha = 1;
    ...
    if (this.st === 'training') {
        ...
        ctx.fillText(`ATK+${safeNum(safeBonus.atk,0).toFixed(3)} RES+${safeNum(safeBonus.res,0).toFixed(3)} SPD+${safeNum(safeBonus.spd,0).toFixed(3)}`, 5, 50);

        if (!this.isInfinite) {
            ctx.fillStyle = '#444'; ctx.fillRect(10, 60, 180, 6);
            ctx.fillStyle = '#0f0'; ctx.fillRect(10, 60, (this.simEpoch / Math.max(1,this.simMaxEpoch)) * 180, 6);
        } else {
        ...
        let drawHpBar = (f, xBase, isP2) => {
            ...
            let barW = hpRatio * 80; let krW = Math.min(80 - barW, krRatio * 80);
            let pCol = f.isAwakened ? (f.awakenColor||'#f00') : (f.color?.body||'#fff');
            
            if (isP2) {
                ctx.fillStyle = '#c0c'; ctx.fillRect(190 - barW - krW, 5, krW, 8);
                ctx.fillStyle = pCol; ctx.fillRect(190 - barW, 5, barW, 8);
            } else {
                ctx.fillStyle = pCol; ctx.fillRect(xBase, 5, barW, 8);
                ctx.fillStyle = '#c0c'; ctx.fillRect(xBase + barW, 5, krW, 8); // カルマは紫色
            }
        };
