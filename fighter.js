...
// === AUTO FIGHTER (Phase 8-2: GAMBIT & COMBO TREE) ===
...
const AutoFighter = {
  ...
  myAI: {
      ...
      // ★ 第2回追加：ガンビットシステム（AIの思考マトリクス）
      gambits: [
          { cond: CondType.HP_LOW, act: ActType.RETREAT }, // 優先度1: ピンチなら下がる
          { cond: CondType.FAR, act: ActType.BEAM },       // 優先度2: 遠ければビーム
          { cond: CondType.AIR, act: ActType.GUARD },      // 優先度3: 敵が飛んだらガード
          { cond: CondType.DEFAULT, act: ActType.COMBO }   // 優先度4: 基本はコンボを狙う
      ],
      
      // ★ 第2回追加：コンボツリー（独自の連携ルート構築）
      comboRoute: ['jab', 'upper', 'meteor'] // ジャブ → アッパー → メテオ
  },
  ...
  update() {
    ...
    if (this.st === 'menu') {
        ...
        if (keysDown.a) {
            playSnd('jmp');
            if (this.menuCur === 0) { alert("バトルシステムは次回（第3回）で完全統合されるぞ！今はAIを育成せい！"); }
            else { this.st = 'lab_main'; this.labCur = 0; }
        }
        return;
    }
    ...
    if (this.st === 'lab_main') {
        // ★ 項目を追加
        const labItems = ['体型＆カラー設定', 'パッシブ＆覚醒', 'ガンビット(思考AI)', 'コンボルート構築', '戻る'];
        if (keysDown.up) { this.labCur = (this.labCur - 1 + labItems.length) % labItems.length; playSnd('sel'); }
        if (keysDown.down) { this.labCur = (this.labCur + 1) % labItems.length; playSnd('sel'); }
        if (keysDown.a) {
            playSnd('hit');
            ...
            else if (this.labCur === 2) { this.st = 'lab_gambit'; this.labCur = 0; }
            else if (this.labCur === 3) { this.st = 'lab_combo'; this.labCur = 0; }
            else { this.st = 'menu'; this.menuCur = 0; }
        }
        if (keysDown.b) { this.st = 'menu'; playSnd('hit'); }
        return;
    ...
    if (this.st === 'lab_awaken') {
        ...
        if (keysDown.right || keysDown.left) {
            ...
            if (this.labCur === 0) {
                let keys = Object.keys(PassiveType); let idx = keys.indexOf(Object.keys(PassiveType).find(k => PassiveType[k] === this.myAI.passive));
                this.myAI.passive = PassiveType[keys[(idx + dir + keys.length) % keys.length]];
            }
            else if (this.labCur === 1) {
                let keys = Object.keys(AwakenCondition); let idx = keys.indexOf(Object.keys(AwakenCondition).find(k => AwakenCondition[k] === this.myAI.awakening.condition));
                this.myAI.awakening.condition = AwakenCondition[keys[(idx + dir + keys.length) % keys.length]];
            }
            else if (this.labCur === 2) {
                let keys = Object.keys(IntroType); let idx = keys.indexOf(Object.keys(IntroType).find(k => IntroType[k] === this.myAI.intro));
                this.myAI.intro = IntroType[keys[(idx + dir + keys.length) % keys.length]];
            }
        }
        return;
    ...
    // --- ★ 第2回追加：ガンビット設定（AIマトリクス） ---
    if (this.st === 'lab_gambit') {
        if (keysDown.up) { this.labCur = (this.labCur - 1 + 4) % 4; playSnd('sel'); }
        if (keysDown.down) { this.labCur = (this.labCur + 1) % 4; playSnd('sel'); }
        if (keysDown.b) { this.st = 'lab_main'; this.labCur = 2; playSnd('hit'); return; }

        if (keysDown.right || keysDown.left) {
            playSnd('sel'); let dir = keysDown.right ? 1 : -1;
            let keys = Object.keys(ActType); 
    ...
    if (this.st === 'lab_combo') {
        if (keysDown.up) { this.labCur = (this.labCur - 1 + 3) % 3; playSnd('sel'); }
        if (keysDown.down) { this.labCur = (this.labCur + 1) % 3; playSnd('sel'); }
        if (keysDown.b) { this.st = 'lab_main'; this.labCur = 3; playSnd('hit'); return; }

        if (keysDown.right || keysDown.left) {
            playSnd('sel'); let dir = keysDown.right ? 1 : -1;
            let keys = Object.keys(SkillDict); 
  ...
  drawPreviewStickman(x, y, aiData, isAwakened = false) {
      let b = aiData.body; let c = isAwakened ? aiData.awakening.color : aiData.color;
      ctx.save(); ctx.translate(x, y); ctx.scale(b.width, b.height); 
      ctx.strokeStyle = c.body; ctx.lineWidth = 3 / Math.max(b.width, b.height); 
      ctx.lineCap = 'round'; ctx.lineJoin = 'round';
      
      let t = Date.now() / 300; let breath = Math.sin(t) * 2;
      let p = { h:{x:0,y:-30+breath}, n:{x:0,y:-20+breath}, hip:{x:0,y:-5}, sL:{x:-10,y:-10+breath}, sR:{x:10,y:-10+breath}, eL:{x:-15,y:5+breath}, eR:{x:15,y:5+breath}, hL:{x:-10,y:15+breath}, hR:{x:10,y:15+breath}, kL:{x:-10,y:10}, kR:{x:10,y:10}, fL:{x:-15,y:25}, fR:{x:15,y:25} };

      ctx.shadowBlur = isAwakened ? 20 + Math.random()*10 : 10; ctx.shadowColor = c.aura;
      ctx.beginPath();
      ctx.moveTo(p.n.x, p.n.y); ctx.lineTo(p.hip.x, p.hip.y); ctx.moveTo(p.n.x, p.n.y); ctx.lineTo(p.sL.x, p.sL.y); ctx.lineTo(p.eL.x, p.eL.y); ctx.lineTo(p.hL.x, p.hL.y); ctx.moveTo(p.n.x, p.n.y); ctx.lineTo(p.sR.x, p.sR.y); ctx.lineTo(p.eR.x, p.eR.y); ctx.lineTo(p.hR.x, p.hR.y); ctx.moveTo(p.hip.x, p.hip.y); ctx.lineTo(p.kL.x, p.kL.y); ctx.lineTo(p.fL.x, p.fL.y); ctx.moveTo(p.hip.x, p.hip.y); ctx.lineTo(p.kR.x, p.kR.y); ctx.lineTo(p.fR.x, p.fR.y); 
      ctx.stroke();
      
      ctx.shadowBlur = 0; ctx.beginPath(); ctx.arc(p.h.x, p.h.y, 8 * b.head, 0, Math.PI * 2); ctx.fillStyle = c.body; ctx.fill();
      ctx.restore();
  },
  ...
  draw() {
    ...
    if (this.st === 'menu') {
        ...
        this.drawPreviewStickman(50, 70, this.myAI, false); this.drawPreviewStickman(145, 70, this.myAI, true);

        ctx.fillStyle = '#0f0'; ctx.font = 'bold 12px monospace';
        
        if (this.st === 'lab_main') {
            ctx.fillText('【CUSTOMIZE MENU】', 30, 140);
            const items = ['体型＆カラー設定', 'パッシブ＆覚醒', 'ガンビット(思考AI)', 'コンボルート構築', '戻る'];
            ctx.font = '10px monospace';
            for(let i=0; i<items.length; i++) {
                ctx.fillStyle = this.labCur === i ? '#ff0' : '#fff';
                ctx.fillText((this.labCur === i ? '> ' : '  ') + items[i], 20, 160 + i * 25);
            }
        }
        else if (this.st === 'lab_body') {
        ...
        // ★ 第2回追加UI：ガンビット
            ...
            for(let i=0; i<4; i++) {
                ctx.fillStyle = this.labCur === i ? '#ff0' : '#888';
                ctx.fillText(`優先度${i+1}: ${this.myAI.gambits[i].cond}`, 10, 160 + i * 30);
                ctx.fillStyle = this.labCur === i ? '#fff' : '#aaa';
                ctx.fillText(` └ 行動: ◀ ${this.myAI.gambits[i].act.padEnd(8, ' ')} ▶`, 10, 172 + i * 30);
            }
            ctx.fillStyle = '#888'; ctx.font = '8px monospace'; ctx.fillText('◀ 左右で行動を変更 ▶', 45, 285);
        }
        // ★ 第2回追加UI：コンボルート
        else if (this.st === 'lab_combo') {
            ctx.fillText('【COMBO ROUTE】', 40, 140); ctx.font = '10px monospace';
            ctx.fillStyle = '#88f'; ctx.fillText('「コンボ始動」時の連携', 25, 155);
            for(let i=0; i<3; i++) {
                ctx.fillStyle = this.labCur === i ? '#ff0' : '#fff';
                let skillName = SkillDict[this.myAI.comboRoute[i]];
