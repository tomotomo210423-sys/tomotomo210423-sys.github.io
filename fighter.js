// === AUTO FIGHTER (Phase 16: BUG-FREE & NEW POWERS EDITION) ===
...
const Styles = {
    ...
    AERO: { name: '空の支配者', desc: '常に空中戦を好む。ジャンプを多用し、上空からの強襲を狙うスタイル。', aggro: 0.6, guard: 0.1, dodge: 0.4, range: 60 } // ★ 新規追加
};
...
const Passives = {
    ...
    HOVER: { name: '浮遊', desc: '重力をほぼ無視して空中に長く滞在できる。空からの攻撃が得意。' } // ★ 新規追加
};
...
const AwakenPassives = {
    ...Passives, // 既存のパッシブも選べる
    CLONE: { name: '影分身', desc: '覚醒時、自身と同じ動きをする分身を2体召喚する。分身は一度被弾すると身代わりになり消滅する。' } // ★ 新規追加
};
...
const Skills = {
    ...
    parapara: {name:'パラパラ', dmg: 5, kb: 1, range: 0, start: 10, act: 5, rec: 20, cd: 90, vx: 0, type: 'summon', desc: '自機を追従するUFOを召喚し、一定時間自動で弾丸を連射させる。'} // ★ 新規追加
};

const SkillKeys = Object.keys(Skills);
const CounterData = {name:'カウンター', dmg: 80, kb: 15, range: 45, start: 4, act: 15, rec: 20, cd: 0, vx: 8, type: 'melee'};

// ★ 安全データ取得ラッパー（NaN汚染をここで遮断）
...
const safeNum = (val, def) => { let n = Number(val); return (isNaN(n) || n === undefined || n === null) ? def : n; };
...
const AutoFighter = {
  ...
  addVFX(type, x, y, color, extra={}) { if(this.isSim) return; this.vfx.push({type, x: safeNum(x,0), y: safeNum(y,0), color: color||'#fff', life: extra.life||20, maxLife: extra.life||20, ...extra}); },
  ...
  createFighter(isP2, data) {
      ...
      return {
          ...
          funnelTimer: 0, hasClones: false // ★ 新規能力用プロパティ
      };
  },
  ...
  update() {
    ...
    // ★ ステータス配分の「小数点の計算誤差バグ」を完全修正（内部で10倍して計算）
  ...
  applyHit(attacker, skill, victim, hx, hy) {
       ...
       let safeWeight = safeNum(victim.physics && victim.physics.weight, 100);
  ...
  processFighter(f, opp) {
    ...
    if (f.funnelTimer > 0) {
        ...
        if (f.funnelTimer % 30 === 0) {
            this.bullets.push({x: f.x, y: f.y - 60, vx: f.dir * 12, vy: (opp.y - (f.y-60))/30, owner: f, skill: {name:'パラ弾', dmg: 5, kb: 1, type: 'shot'}, life: 60});
            this.addVFX('impact', f.x, f.y-60, '#0ff', {size: 15, life: 5});
        }
    }

    // ★ 持続フレームすり抜け防止の神対応（状態変化でリセット）
    if (f.state !== f.prevState) { f.hasHit = false; f.prevState = f.state; }

    f.stateFrame++; 
    let safeSpd = safeNum(f.base && f.base.spd, 1.0);
    ...
    if (!f.isAwakened) {
        ...
        if (awaken) {
            ...
            if (f.awakenPassive === 'CLONE') f.hasClones = true; // 影分身発動！
    ...
    let safeWeight = safeNum(f.physics && f.physics.weight, 100);
    
    // ★ 浮遊パッシブの物理演算
    let grav = pKey === 'HOVER' ? 0.05 : 0.6;
    if (f.y < this.groundY) { f.vy += grav + (safeWeight - 100)*0.005; } else { f.y = this.groundY; f.vy = 0; }
    ...
    if (isAtk && skill) {
         ...
         if (f.stateFrame >= sFrame && f.stateFrame <= sFrame + actFrame) { 
             if (f.stateFrame === sFrame) {
                 ...
                 if (sType === 'summon') { f.funnelTimer = 180; this.play('jmp'); } // パラパラ発動！
    ...
    if ((f.state === 'idle' && f.cd <= 0) || canCancel) {
        ...
        if (opp.state === 'hurt' || opp.state === 'stunned' || opp.state === 'knockdown') {
            ...
            // ★ 空の支配者（AERO）ならジャンプで近付く
            if (st.name === '空の支配者' && !isAir && Math.random() < 0.2 && !canCancel) { f.state = 'move'; f.stateFrame = 0; f.vy = -18; f.vx = f.dir * 6; return; }
        ...
        if (shouldAttack) {
            ...
            let safeWidth = safeNum(f.body&&f.body.width, 1.0);
            ...
            for (let skey of sKeys) {
                ...
                if (sType === 'summon' && f.funnelTimer <= 0) score += 30; // パラパラ撃つ
  ...
  createHitbox(attacker, skill, victim) {
    ...
    let actualRange = safeNum(skill.range, 30) * safeNum(attacker.body&&attacker.body.width, 1.0);
  ...
  drawStickman(f, alpha = 1, isTrail = false) {
    if (!f) return; 
    let bW = safeNum(f.body&&f.body.width, 1.0); let bH = safeNum(f.body&&f.body.height, 1.0); let bHead = safeNum(f.body&&f.body.head, 1.0);
    ...
    let drawCore = (dx, dy, myAlpha) => {
        ...
        if (fst === 'idle') { let t = Date.now()/200; p.h.y+=Math.sin(t); p.n.y+=Math.sin(t); p.hL.y+=Math.sin(t+1)*2; p.hR.y+=Math.sin(t+1)*2; p.hip.y+=2; p.kL.x-=2; p.kR.x+=2; p.kL.y-=2; p.kR.y-=2; } 
        else if (fst === 'move') { if (f.y < this.groundY - 5) { p.hL.y-=20; p.hR.y-=20; p.eL.y-=10; p.eR.y-=10; p.kL.y-=15; p.kR.y-=5; p.fL.y-=15; p.h.x+=5; } else { let run = Math.sin(sf * 0.6) * 15; p.hL.x=-run; p.hR.x=run; p.eL.x=-run/2; p.eR.x=run/2; p.fL.x=run; p.fR.x=-run; p.kL.x=run/2; p.kR.x=-run/2; p.h.x+=5; p.n.x+=5; p.hip.x+=3; } }
    ...
    // ★ 影分身の描画
    if (f.hasClones && !isTrail) {
        drawCore(dx - 40, dy, 0.4);
        drawCore(dx + 40, dy, 0.4);
    }
