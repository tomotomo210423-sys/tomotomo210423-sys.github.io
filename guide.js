// === GUIDE ROOM (図書館) - DESIGN RESTORED ===
const Guide = {
  ...
  books: [
    ...
    {x: 7, y: 5, name: '【RPG】', msg: '--- 概要 ---\n広大な世界を冒険するRPG。\n敵はフィールドをうろついている。\n工房での武器強化や\nカスタム闘技場も完備！'},
    {x: 2, y: 8, name: '【ONLINE】', msg: '--- 概要 ---\n白熱の擬似オンライン対戦！\nパドルを操作して\nボールを打ち返せ。\n先に5点先取で勝利だ。'},
    {x: 7, y: 8, name: '【引継ぎ】', msg: '--- セーブの永久保存 ---\nメニューの「データ引継ぎ」で\n秘密の管理室に行ける。\n端末を操作してデータを\nパスワード化しよう！'}
  ],
  ...
  update() {
    ...
    for(let r=0; r<15; r++) {
      for(let c=0; c<10; c++) {
        // ★ 直りました！ 1(壁)以外の場所を歩けるように修正
        if(this.map[r][c] !== '0' && this.map[r][c] !== '2' && this.map[r][c] !== '3' && this.map[r][c] !== '4' && this.map[r][c] !== '5' && this.map[r][c] !== '6' && this.map[r][c] !== '7') {
