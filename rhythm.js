...
const Rhythm = {
  ...
  generateNotes(buffer) {
    ...
    if (this.notes.length < 10) {
       ...
       for (let t = 2; t < buffer.duration; t += minGap * 1.5) {
           let isL = Math.random() < 0.2; let dur = isL ? 0.4 + Math.random() * 0.6 : 0;
           this.notes.push({ time: t, lane: Math.floor(Math.random() * 4), hit: false, y: -50, missed: false, type: isL?'long':'short', dur: dur, hold: false });
       }
    }
    this.startPlay();
  },
  ...
  hitKey(lane) {
      ...
      if (hitNote) {
        ...
        if(hitNote.type === 'long') {
           hitNote.hold = true;
           addParticle(cx, this.lineY, this.colors[lane], 'star'); playSnd('hit');
  ...
  update() {
    ...
    if (this.st === 'menu') {
      ...
      let isP = [
          k.left || k.l0 || this.laneTouch[0],
          k.down || k.l1 || this.laneTouch[1],
          k.up   || k.l2 || this.laneTouch[2],
          k.right|| k.l3 || this.laneTouch[3]
      ];
           ...
           if (n.hold) {
              if (isP[n.lane]) {
                 ...
                 if (endY > this.lineY) {
                    ...
                    this.combo++; if(this.combo > this.maxCombo) this.maxCombo = this.combo;
                    this.judgements.push({ msg: 'PERFECT', life: 30, color: '#ff0', lane: n.lane });
                    playSnd('combo');
                 }
              } else {
                 n.hold = false; n.missed = true; // 途中で離してもペナルティなしで消える
              }
  ...
  draw() {
    ...
    if (this.st === 'menu') {
      ...
      for (let i = 0; i < 3; i++) {
        ctx.fillStyle = this.mode === modes[i] ? '#0f0' : '#666'; ctx.fillText((this.mode === modes[i] ? '> ' : '  ') + modes[i].toUpperCase(), 65, 125 + i * 35);
        ctx.fillStyle = this.mode === modes[i] ? '#ff0' : '#888'; ctx.font = '8px monospace'; ctx.fillText(`HI-SCORE: ${rData[modes[i]] || 0}`, 65, 137 + i * 35); ctx.font = '10px monospace'; 
      }
